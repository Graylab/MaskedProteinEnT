...
class PPISequenceWriter():
    def __init__(self, outdir, chain_breaks_dict, partner_selection, lengths_dict):
        self.outdir = outdir
    ...
    def write_sequences(self, recovery_dict, partner_name, write_fasta_for_colab_sampled=False, 
        ...
        total_nodes = recovery_dict['total_nodes']
        argmax_loss = recovery_dict['argmax_loss']
            
        comma_separated_outstr = f'pdbid={cleanid},total_nodes={total_nodes},score={{}},'
        ...
        for j, (seq, rec, seq_loss) in enumerate(zip(sequences_sampled, seqrec_sampled, sequences_sampled_loss)):
            outstr += f'>seq{j},T={temp},'+comma_separated_outstr.format(seq_loss, rec)+'\n'
        ...
        if not (write_fasta_for_colab_sampled or write_fasta_for_colab_argmax):
            ...
            outfile_sampled = f'{outdir}/{cleanid}_sequences_sampled_temp{temp}_N{n_seqs}_{partner_name}.fasta'
        ...
        if write_fasta_for_colab_sampled or write_fasta_for_colab_argmax:
            ...
            outfile_json = f'{self.outdir}/{cleanid}_sequence_recovery_argmax_{partner_name}.json'
            print(seqrec_argmax)
            json.dump(seqrec_argmax, open(outfile_json, 'w'))
