...
def _helper_loader(dataset, args, with_metadata=False):
    ...
    if not with_metadata:
        ...
        collate_fn=SCNProteinMaskedMultiAtomDatasetBatched.merge_samples_to_minibatch,
        ...
        dataset,
        shuffle=args.shuffle_dataset,
        collate_fn=SCNProteinMaskedMultiAtomDatasetBatched.merge_samples_to_minibatch_with_metadata,
        batch_size=args.batch_size,
        pin_memory=True,
        num_workers=args.num_procs)
    return loader
...
class MaskedSequenceStructureMADataModule(pl.LightningDataModule):
    ...
    def test_dataloader(self):
        ...
        if is_protein_dataset(args):
            #print('USE SCN')
            if self.test_protein_dataset is None:
                print('SCN file not found! or --h5_file_protein not found')
                sys.exit()
